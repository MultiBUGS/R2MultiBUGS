findMultiBUGS <- function(){
  ...
  ob.reg <- try(utils::readRegistry("Software\\MultiBUGS",
                                    "HLM",
                                    view = "32-bit"),
                silent = TRUE)
  ...
  if (length(version.inst > 1)){
    id <- which(apply(outer(version.inst,
                            version.inst,
                            Vectorize(compareVersion, c("a", "b"))),
                      1,
                      function(x) all(x >= 0)))
  ...
  if (compareVersion(max(version.inst), version.req) < 0){
    warning("Found MultiBUGS version ",
            version.inst,
            ".\n Requires ",
            version.req,
            " or greater.")
  ...
  utils::readRegistry(paste("Software",
                            "MultiBUGS",
                            rnames,
                            sep = "\\"),
                      "HLM",
                      view = "32-bit")[["InstallPath"]]
}
