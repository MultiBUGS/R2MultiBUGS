%\VignetteIndexEntry{R2MultiBUGS}
\documentclass{Z}
\usepackage{RdRW,thumbpdf}
\newcommand{\WinBUGS}{{\proglang{WinBUGS}}{}}
\newcommand{\MultiBUGS}{{\proglang{MultiBUGS}}{}}
\newcommand{\RW}{{\pkg{R2WinBUGS}}{}}
\newcommand{\RO}{{\pkg{R2MultiBUGS}}{}}
\renewcommand{\R}{{\proglang{R}}{}}
\setlength{\textheight}{23.5cm}

\title{\RO{}:\protect\linebreak A Package for Running \MultiBUGS{} from \R{}}
...
\Plaintitle{R2MultiBUGS: A Package for Running MultiBUGS from R}

\Abstract{
The \RO{} package provides convenient functions to call \MultiBUGS{} from \R{}. It
automatically writes the data and scripts in a format readable by \MultiBUGS{} for processing in batch mode, which
is possible since version 1.4. After the \MultiBUGS{} process has finished, it is possible either to read the
...
\Keywords{\R{}, \MultiBUGS{}, interface, MCMC}
\Plainkeywords{R, MultiBUGS, interface, MCMC}
...
The usage of Markov chain Monte Carlo (MCMC) methods became very popular within the last decade. \MultiBUGS{}
...
distributions.
The \MultiBUGS{} software is available for free
at \url{http://www.MultiBUGS.info}.
An introduction to MCMC methods is given in \cite{Gilks;Richardson;Spiegelhalter:1996}.

Using \MultiBUGS{}, the user must specify the model to run, and to load data and initial values for a specified
...
\MultiBUGS{} comes with the ability to run the software in batch mode using scripts.

The \RO{} package makes use of this feature and provides the tools to call \MultiBUGS{}
...
rather than using ``copy \& paste'' to update data in \MultiBUGS{} each time; however difficulties have been encountered in this area because both \R{} and \MultiBUGS{} can lock up RAM in the Windows operating system.
...
\verb+install.packages("R2MultiBUGS")+ at the \R{} command prompt.
Do not forget to load the package with \verb+library("R2MultiBUGS")+.

The package \pkg{coda} by \cite{Plummer;Best;Cowles;Vines:2004} is very useful for the analysis of \MultiBUGS{}'
...
script in a format readable by \MultiBUGS{} for processing in batch mode, and call \MultiBUGS{} from \R{}. After the
\MultiBUGS{} process has finished, it is possible either to read the resulting data into \R{} by the package itself
...
We model these data by Poisson-Gamma models introduced by \cite{Best;Ickstadt;Wolpert:2000} using \MultiBUGS{}.
...
Unfortunately, \MultiBUGS{} does not support a list format such as directly produced by \R{}.
...
Unfortunately, opening a file of such size really slows \MultiBUGS{} down,
...
probably due to memory management issues in \MultiBUGS{}.
...
\begin{enumerate}
    \item \verb+bugs.data.inits()+ writes the data files \file{data.txt}, and \file{inits1.txt}, \file{inits2.txt}, ...
        into the working directory. These files will be used by \MultiBUGS{} during batch processing.

        In particular, input for \MultiBUGS{} must not exceed a certain number of digits.
    ...
    \item \verb+bugs.script()+ writes the file \file{script.txt} that is used by \MultiBUGS{} for batch processing.
    ...
    \item \verb+bugs.sims()+ is only called if the argument \verb+codaPkg+ has been set to \verb+FALSE+ (the default).
        ...
        The function \verb+bugs.sims()+ reads simulations from \MultiBUGS{} into \R{} (not necessarily called by \verb+bugs()+
        itself), formats them,
...
%\newpage
As known from \MultiBUGS{}, one must specify the \verb+data+ in form of a list, with list names equal to the names
of data in the corresponding \MultiBUGS{} model. Alternatively, it is possible to specify a vector or list of names
...
\verb+bugs()+ just uses the starting values created by \MultiBUGS{}; but in practice \MultiBUGS{} can crash when
...
\MultiBUGS{} remains open after the run.
...
%% Revision 2005-01-10 start
Since the communication between \MultiBUGS{} and \R{} is based on files,
...
variances of the deviance.\footnote{In contrast, \cite{Spiegelhalter;Best;Carlin;vdLinde:2002},
 and MultiBUGS, define $p_D$ as the
posterior mean of the deviance evaluated at the posterior mean of the parameter values.  We cannot use that
definition because the deviance function is not available to our program, which calls \MultiBUGS{} from the
...
we choose this measure purely because it is computationally possible given what is available to us from the \MultiBUGS{} output.}
...
For an example, see Figure~\ref{plot} in Section~\ref{School2}.  It is intended to adapt this function to work with MCMC output in general, even if obtained from software other than MultiBUGS.
...
their prior distributions, see the following \MultiBUGS{} code:
...
parameters.  As can be seen in the right column, \RO{} uses the parameter names in \MultiBUGS{} to structure the
...
Since we want to use the \pkg{coda} package for reading the data into \MultiBUGS{}, we specify \verb+codaPkg = TRUE+
